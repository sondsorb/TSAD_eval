...
def distance_problem():
    ...
    metrics = ["AF"]
    results = [[0.7],[.1]] # NOTE not correct!
...
def create_table(anomalies, metric_list, length, name = None, scale=None):
    results = [[metric(length, anomalies[0], predicted_anomalies).get_score() for metric in metric_list] for predicted_anomalies in anomalies[1:]]
...
def make_figure_content(length, anomalies): # TODO cleanup this (this function only exist because "anomalies" in makefig is a mess
    figure_anomalies = []
    for ts in anomalies:
